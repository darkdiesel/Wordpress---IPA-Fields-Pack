...
if ( ! class_exists( 'IPA_Fields_Pack_Admin_Menu' ) ):
	class IPA_Fields_Pack_Admin_Menu {
		...
		public function admin_menu() {
		  ...
		  $this->pages_hooks['taxonomy_list_page_hook'] = add_submenu_page( 'ipa-fields-pack', __( 'Taxonomies' ), __( 'Taxonomies' ), 'manage_options', 'ipa-fields-pack', array(
			'IPA_Fields_Pack_Admin_Menu_User_Coupons',
			'admin_menu_output'
		  ) );

			$this->pages_hooks['settings_page_hook'] = add_submenu_page( 'ipa-fields-pack', __( 'Settings' ), __( 'Settings' ), 'manage_options', 'ipa-fields-pack-settings', array(
		...
		public function admin_menu_settings_scripts( $hook ) {
			...
			if ($this->pages_hooks['taxonomy_list_page_hook'] == $hook){
			  IPA_Fields_Pack_Admin_Menu_User_Coupons::admin_menu_scripts($plugin_data);
			}
		}
